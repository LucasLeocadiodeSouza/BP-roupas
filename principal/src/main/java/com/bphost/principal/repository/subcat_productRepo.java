...
public interface subcat_productRepo extends JpaRepository<subcat_product, subcat_productId>{
    ...
    @Query("SELECT scprod FROM subcat_product scprod WHERE scprod.id.category_id = :categ_id AND scprod.id.subcategory_seq = :subcateg_seq")
    List<subcat_product> findAllSubcategProdBySubCategory(@Param("categ_id") Integer categ_id, @Param("subcateg_seq") Integer subcateg_seq);
}
