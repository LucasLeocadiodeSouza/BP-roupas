...
public interface userListRepo extends JpaRepository<userList, userListId>{
    ...
    @Query("SELECT list FROM userList list WHERE list.id.useraccount_id = :userId AND list.name = :name")
    userList findUserListByName(@Param("userId") Integer userId, @Param("name") String name);
